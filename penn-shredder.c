...
char *getCommandFromInput() {
    char* user_input = (char*) malloc(1024); // save user input to heap
    char* ptr1 = user_input;
    char* ptr2 = user_input;
    //read user input smaller than 1023 characters  from keyboard 
    ssize_t read_input = read(0, user_input, 1023); 
//On success, the number of bytes read is returned (zero indicates end of file)
    if( read_input == 0){
        free(user_input);
        exit(EXIT_SUCCESS);
    }else if(read_input == -1){ // invalid input 
        free(user_input);
        exit(EXIT_FAILURE);
    }

    while(ptr1 < user_input + 1023){ // remove extra white space
        if(*ptr1 == '\n'){ //"\n" = enter,  newline character
            *ptr2 = '\0';
            break;
        }else if(*ptr1 != 32){ // 32 is space in ASCII 
            *ptr2 = *ptr1;
            ptr2++;
        }
